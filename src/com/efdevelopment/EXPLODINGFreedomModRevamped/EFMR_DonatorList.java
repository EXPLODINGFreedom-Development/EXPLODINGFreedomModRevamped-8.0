...
import com.efdevelopment.EXPLODINGFreedomModRevamped.EFMR;
...
public class EFMR_DonatorList
{
    private static final Map<UUID, EFMR_Donator> donorList;
    ...
    public static Set<String> getDonatorNames()
    {
        ...
        for (EFMR_Donator admin : donorList.values())
        {
    ...
    public static Set<EFMR_Donator> getAllAdmins()
    {
    ...
    public static void load()
    {
        ...
        final TFM_Config config = new TFM_Config(TotalFreedomMod.plugin, EFMR.DONATOR_FILE, true);
        config.load();
        ...
        for (String uuidString : section.getKeys(false))
        {
            ...
            final EFMR_Donator donor = new EFMR_Donator(uuid, section.getConfigurationSection(uuidString));
    ...
    {
        final File oldYaml = new File(TotalFreedomMod.plugin.getDataFolder(), EFMR.DONATOR_FILE);
        final File newYaml = new File(TotalFreedomMod.plugin.getDataFolder(), EFMR.DONATOR_FILE + ".bak");
    ...
    public static void updateIndexLists()
    {
        ...
        EFMR_DonatorWorld.getInstance().wipeAccessCache();
    }
    ...
    public static void save()
    {
        ...
        Iterator<Entry<UUID, EFMR_Donator>> it = donorList.entrySet().iterator();
        while (it.hasNext())
        {
            Entry<UUID, EFMR_Donator> pair = it.next();

            UUID uuid = pair.getKey();
            EFMR_Donator donor = pair.getValue();
    ...
    public static EFMR_Donator getEntry(Player player)
    {
    ...
    public static EFMR_Donator getEntry(UUID uuid)
    {
        return donorList.get(uuid);
    }

    @Deprecated
    public static EFMR_Donator getEntry(String name)
    {
    ...
    public static EFMR_Donator getEntryByIp(String ip)
    {
        return getEntryByIp(ip, false);
    }

    public static EFMR_Donator getEntryByIp(String needleIp, boolean fuzzy)
    {
        Iterator<Entry<UUID, EFMR_Donator>> it = donorList.entrySet().iterator();
        while (it.hasNext())
        {
            final Entry<UUID, EFMR_Donator> pair = it.next();
            final EFMR_Donator donor = pair.getValue();
    ...
    {
        final EFMR_Donator admin = getEntry(player);
        if (admin == null)
        {
            return;
        }
    ...
    public static boolean isSeniorDonor(CommandSender sender, boolean verifySuperadmin)
    {
        ...
        final EFMR_Donator entry = getEntry((Player) sender);
        if (entry != null)
        {
    ...
    public static boolean isIdentityMatched(Player player)
    {
        ...
        final EFMR_Donator entry = getEntry(player);
        if (entry == null)
        {
            return false;
        }
    ...
    public static boolean checkPartialSuperadminIp(String ip, String name)
    {
        ...
        try
        {
            ...
            if (matchIp != null)
            {
                final EFMR_Donator entry = getEntryByIp(matchIp);
    ...
    public static void addSuperadmin(OfflinePlayer player)
    {
        ...
        if (donorList.containsKey(uuid))
        {
            final EFMR_Donator donor = donorList.get(uuid);
        ...
        final EFMR_Donator donor = new EFMR_Donator(
                uuid,
                player.getName(),
                new Date(),
                "",
