...
public final class EFMR_BuilderWorld extends TFM_CustomWorld
{
        ...
        if (world != null && event.getTo().getWorld() == world)
        {
            ...
            if (!canAccessWorld(player))
            {
                ...
                if (lastTP == null || lastTP.longValue() + TP_COOLDOWN_TIME <= currentTimeMillis)
                {
                    ...
                    TFM_Log.info(player.getName() + " attempted to access the Builder's World.");
                    new BukkitRunnable()
                    {
                        @Override
                        public void run()
    ...
    public static EFMR_BuilderWorld getInstance()
    {
        return EFMR_BuilderWorldHolder.INSTANCE;
    }

    private static class EFMR_BuilderWorldHolder
    {
        private static final EFMR_BuilderWorld INSTANCE = new EFMR_BuilderWorld();
    }
}